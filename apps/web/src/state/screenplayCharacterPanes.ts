...
import { normalizeCharacterCueText } from './screenplayCharacterLookup';
...
export interface CharacterPaneState {
  /** normalizeCharacterCueText(name) — same key as useScreenplayCharacterLookupStore's charactersByName. */
  id: string;
  /** Display name, as typed in the document's character cue. */
...
interface ScreenplayCharacterPanesState {
  panes: Record<string, CharacterPaneState>;
  nextZIndex: number;
  openPane: (characterName: string, anchor?: { x: number; y: number }) => void;
...
export const useScreenplayCharacterPanesStore = create<ScreenplayCharacterPanesState>()((set, get) => ({
  panes: {},
  nextZIndex: BASE_Z_INDEX,
  openPane: (characterName, anchor) => {
    const id = normalizeCharacterCueText(characterName);
    ...
    set({
      panes: { ...panes, [id]: { id, characterName, x, y, zIndex: nextZIndex } },
      nextZIndex: nextZIndex + 1,
    });
  },
  closePane: (id) => {
