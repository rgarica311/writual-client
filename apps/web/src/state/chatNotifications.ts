...
export const useChatNotificationsStore = create<ChatNotificationsState>()(
  persist(
    ...
    {
      name: STORAGE_KEY,
      // Only the preference survives a reload. The open conversation belongs to this tab and this
      // moment, and the subscription is re-read from the browser on every start — restoring either
      // from storage would act on something that may no longer be true.
